...
def test_successful_mapping_and_coercion(client, mocker):
...
def test_coercion_with_dirty_strings(client, mocker):
...
def test_unrecognized_number(client, mocker):
...
def test_swapi_returns_error(client, mocker):
