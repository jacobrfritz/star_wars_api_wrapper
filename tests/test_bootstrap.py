...
def test_rename_project(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    pyproject.write_text(
        ...
        "\n"
        "[project.scripts]\n"
        'base-project = "base_fast_api.cli:main"',
        encoding="utf-8",
    )
    ...
    src_dir = tmp_path / "src" / "base_fast_api"
    src_dir.mkdir(parents=True)
    (src_dir / "__init__.py").touch()

    main_py = src_dir / "main.py"
    main_py.write_text('print("base-fast-api")', encoding="utf-8")

    tests_dir = tmp_path / "tests"
    tests_dir.mkdir()
    test_main_py = tests_dir / "test_main.py"
    test_main_py.write_text(
        'from base_fast_api import main\nassert "base-fast-api" == "base-fast-api"',
        encoding="utf-8",
    )
